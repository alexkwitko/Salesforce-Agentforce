...
**Layer 2 — deterministic hard stop (free, optional).** An Apex **trigger** on inbound messages (`ConversationEntry`) counting messages per session/time window; on flood, **end the conversation** (close the `MessagingSession`). A trigger is platform Apex — **no Flex cost**. Caveat: a flag set mid-session can't be read back via a **linked** agent variable (those resolve at session START), so the trigger must **act** (end/close) rather than rely on the agent re-reading a variable.

**Layer 3 — edge controls (the real flood defense).** Client-side throttle in the embedded snippet (disable send while the agent responds, debounce rapid sends) + **CAPTCHA / WAF rate-limit** on the site so automated floods never reach the channel. Salesforce will not cool down the user for you.
