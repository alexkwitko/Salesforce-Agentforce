...
## Drafting a plain `Quote` + `QuoteLineItem` (the no-engine "Asset-path" quoting) — RLM line-item gotchas (hard-won)
When the Place Quote engine is gated, an agent can still draft a STANDARD `Quote` (`OpportunityId` required → auto-create/reuse an open Opp; `Pricebook2Id` = standard; then `QuoteLineItem`). But once **Revenue Cloud is enabled in the org, `QuoteLineItem` inserts get RLM validations even for plain quotes** — discovered the hard way (one error per deploy):
- **The product must be related to a `ProductSellingModel`** or the line throws *"this product must be related to a product selling model."* Filter product selection to `Product2Id IN (SELECT Product2Id FROM ProductSellingModelOption)` — only those are quotable. (In a coffee org, only the *subscription* products had selling models; the one-time SKUs did not.)
- **`ProductSellingModelId` and `PricingTermCount` are READ-ONLY** on `QuoteLineItem` (*"Field … is not editable"*) — the model is auto-derived from the product; don't set them.
- **Subscription selling models add required fields:** `BillingFrequency` can't be null when SellingModelType is **Evergreen or Term-Defined**; **Term-Defined also needs `StartDate` + (`EndDate` or `SubscriptionTerm`)**. Set `BillingFrequency` (e.g. `'Monthly'`), `StartDate=today`, and `SubscriptionTerm`/`EndDate` for a term. Set them **dynamically** (`qli.put('BillingFrequency', …)`) guarded by `SObjectType…fields.getMap().containsKey(...)` so the class still compiles on orgs without Revenue Cloud fields.
- Wrap Quote-header + line insert in a **savepoint/rollback** so a failing line never leaves an orphan Quote header.
- **Agent UX:** make every action input optional and default sensibly (account → most-recent, product → a quotable default, qty → 1) and instruct the agent to **call the action immediately, never interrogate** the user for quote name / opportunity / pricebook / addresses / expiration / status — the action fills those in. See `salesforce-agentforce` skill. (Full working impl: `QuoteDraftService`/`DraftQuoteAction` in the reference repo.)
