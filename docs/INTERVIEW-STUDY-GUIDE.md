...
**The test trick (no widget needed):** from the storefront console, read the `JWT` from `localStorage["00Dfj00000QEVrQ_WEB_STORAGE"]`, `POST {scrt2}/iamessage/v1/conversation` with `{conversationId, routingAttributes:{}}` and a Bearer token, open the SSE stream at `/eventrouter/v1/sse`, and read `CONVERSATION_MESSAGE` events from role=Chatbot. Lets you drive the live agent programmatically.
