...
EMAIL = __import__("sys").argv[1] if len(__import__("sys").argv)>1 else "bea@bluebottledemo.com"
...
st,b=post("/iamessage/v1/conversation/"+CONV+"/message",
          {"id":str(uuid.uuid4()),"messageType":"StaticContentMessage","staticContent":{"formatType":"Text","text":"Is my espresso machine covered for maintenance, and can you book a service visit for 2026-10-20?"}},tok)
print("send:",st)
def agent_texts():
    out=[]
    for e,d in events:
...
while time.time() < _deadline:
    if [x for x in agent_texts() if x[0] == AGENT_NAME and not x[1].startswith("Welcome")]:
        break
    time.sleep(5)
print("=== TRANSCRIPT ===")
