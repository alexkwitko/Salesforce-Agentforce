...
(function () {
  ...
  // to /identify keyed by this browser's cart token. We poll it and fire the native Data Cloud identify
  // path so this device gets stitched to the verified email.
  if (CFG.identifyUrl) {
  ...
  function pollIdentify() {
    fetchJSON(CFG.identifyUrl + "?token=" + encodeURIComponent(token()))
      .then(function (q) {
        ...
        fetch(CFG.identifyUrl + "?token=" + encodeURIComponent(token()), {
          method: "DELETE"
        }).catch(function () {});
      })
      .catch(function () {});
