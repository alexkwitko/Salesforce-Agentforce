  ...
  function setHiddenPrechatFields() {
    try {
      if (
        ...
        var fields = { Kwitko_Cart_Token__c: token() };
        if (CFG.loggedIn && CFG.email) {
          // Send both names: Salesforce hidden pre-chat uses channel variables,
          // while linked agent variables read the MessagingSession field API names.
          fields.Kwitko_Logged_In_Email__c = CFG.email;
          fields.loggedInEmail = CFG.email;
          if (CFG.firstName) {
            fields.Kwitko_Logged_In_First_Name__c = CFG.firstName;
            fields.loggedInFirstName = CFG.firstName;
          }
        }
        embeddedservice_bootstrap.prechatAPI.setHiddenPrechatFields(fields);
      }
    } catch (e) {
      /* hidden prechat optional */
    }
  }
